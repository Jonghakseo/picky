...
enum PickyWorkspaceSeeder {
    ...
    static func seedDefaultWorkspace(
        ...
        migrateLegacyGeneratedAgentsMarkdown(
    ...
    /// Replaces only the exact default written by the retired plan-narration
    /// release. Files in the default workspace are user-owned after seeding, so
    ...
    private static func migrateLegacyGeneratedAgentsMarkdown(
        ...
        guard fileManager.fileExists(atPath: agentsURL.path),
              let existing = try? Data(contentsOf: agentsURL),
              existing == Data(legacyAgentsMarkdownWithTellPlan.utf8)
        else { return }

        do {
            try defaultAgentsMarkdown.write(to: agentsURL, atomically: true, encoding: .utf8)
            log("🔁 Picky: Migrated legacy \(agentsMarkdownFilename) to the current default")
        } catch {
