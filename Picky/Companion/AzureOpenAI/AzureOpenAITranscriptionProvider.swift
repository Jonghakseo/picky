...
final class AzureOpenAITranscriptionProvider: BuddyTranscriptionProvider {
    ...
    /// Transcription steering prompt. Branches on the user's chosen Picky
    /// language so the Korean version is sent when the user speaks Korean and
    /// the English version when they speak English. Either prompt mentions
    /// the product brand names verbatim so the model doesn't translate them.
...
private final class AzureOpenAITranscriptionSession: BuddyStreamingTranscriptionSession {
    ...
    func requestFinalTranscript() {
        ...
        PickyLog.notice(
            ...
            message: "event=sttRequestStarted provider=azure audioBytes=\(audioByteCount)"
        )

        transcriptionTask = Task { [configuration, transcriptionURL, preferredLanguage, transcriptionPrompt, urlSession, onFinalTranscriptReady, requestStartedAt, audioByteCount] in
            do {
                ...
                PickyLog.notice(
                    .latency,
                    prefix: "⏱️ Picky latency —",
                    message: "event=sttRequestFinished provider=azure ms=\(requestMilliseconds) audioBytes=\(audioByteCount) chars=\(transcript.count)"
                )
                ...
                PickyLog.notice(
                    .latency,
                    prefix: "⏱️ Picky latency —",
                    message: "event=sttRequestFailed provider=azure ms=\(requestMilliseconds) audioBytes=\(audioByteCount)"
                )
                guard !Task.isCancelled else { return }
