...
struct AppleSpeechRecognitionState {
    private var latestCompleteResult = ""

    /// `SFSpeechRecognitionResult.bestTranscription` is the recognizer's
    /// complete hypothesis for the current request. Partial callbacks revise
    /// that hypothesis, so they replace the previous value instead of being
    /// concatenated with it. Keeping only the last nonempty result also gives
    /// the finalization error fallback a usable transcript.
    mutating func update(with recognizedText: String) -> String {
        let completeResult = recognizedText.trimmingCharacters(in: .whitespacesAndNewlines)
        if !completeResult.isEmpty {
            latestCompleteResult = completeResult
        }
        return latestCompleteResult
    }
}
...
private final class AppleSpeechTranscriptionSession: NSObject, BuddyStreamingTranscriptionSession {
    ...
    private var recognitionState = AppleSpeechRecognitionState()
    ...
    private func handleRecognitionEvent(
        ...
        if let result {
            latestRecognizedText = recognitionState.update(with: result.bestTranscription.formattedString)
