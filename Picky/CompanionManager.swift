...
final class CompanionManager: ObservableObject {
    ...
    you're picky, a friendly always-on companion that lives in the user's menu bar. the user just spoke to you via push-to-talk and you can see their screen(s). your reply will be spoken aloud via text-to-speech, so write the way you'd actually talk. this is an ongoing conversation — you remember everything they've said before.
            ...
            do {
                ...
                if let pointCoordinate = parseResult.coordinate,
                   let targetScreenCapture {
                    ...
                    PickyAnalytics.trackElementPointed(elementLabel: parseResult.elementLabel)
                ...
                PickyAnalytics.trackAIResponseReceived(response: spokenText)
                ...
                if !spokenText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                    do {
                        ...
                        PickyAnalytics.trackTTSError(error: error.localizedDescription)
    ...
    func setupOnboardingVideo() {
        ...
        // Picky flies to something interesting on screen and comments on it
        let demoTriggerTime = CMTime(seconds: 40, preferredTimescale: 600)
        onboardingDemoTimeObserver = player.addBoundaryTimeObserver(
            forTimes: [NSValue(time: demoTriggerTime)],
            queue: .main
        ) { [weak self] in
            PickyAnalytics.trackOnboardingDemoTriggered()
        ...
        onboardingVideoEndObserver = NotificationCenter.default.addObserver(
            ...
            guard let self else { return }
            PickyAnalytics.trackOnboardingVideoCompleted()
            self.onboardingVideoOpacity = 0.0
