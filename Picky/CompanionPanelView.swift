...
struct CompanionPanelView: View {
    ...
    private var permissionsCopySection: some View {
        if companionManager.hasCompletedOnboarding && companionManager.allPermissionsGranted {
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text("Hi, I'm Farza. This is Picky.")
                    .font(.system(size: 12, weight: .bold))
                    .foregroundColor(DS.Colors.textSecondary)
                ...
                Text("Nothing runs in the background. Picky will only take a screenshot when you press the hot key. So, you can give that permission in peace. If you are still sus, eh, I can't do much there champ.")
                    .font(.system(size: 11))
    ...
    // MARK: - Show Picky Cursor Toggle

    private var showPickyCursorToggleRow: some View {
        HStack {
            HStack(spacing: 8) {
            ...
            Toggle("", isOn: Binding(
                get: { companionManager.isPickyCursorEnabled },
                set: { companionManager.setPickyCursorEnabled($0) }
            ))
            .toggleStyle(.switch)
            .labelsHidden()
