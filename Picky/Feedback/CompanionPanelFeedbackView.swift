...
enum PickyFeedbackMessageTextSynchronization {
...
private struct PickyFeedbackMessageTextView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSScrollView {
        ...
        let textView = PickyFeedbackMessageNSTextView()
    ...
    func updateNSView(_ scrollView: NSScrollView, context: Context) {
        guard let textView = scrollView.documentView as? PickyFeedbackMessageNSTextView else { return }
        context.coordinator.text = $text

        // Match the HUD composer: never push SwiftUI's binding value back into
        // AppKit while an IME marked-text composition is active. SwiftUI can
        // re-render the feedback form before the first Korean syllable commits;
        // assigning `string` at that moment drops the marked text.
        if PickyFeedbackMessageTextSynchronization.shouldOverwriteNativeText(
    ...
    private func applyConfiguration(to textView: PickyFeedbackMessageNSTextView) {
        textView.font = font
...
private final class PickyFeedbackMessageNSTextView: NSTextView {
    private var isCommittingMarkedTextWithReturn = false

    override func keyDown(with event: NSEvent) {
        // Match the HUD composer: while an IME composition is active, let the
        // input context commit/cancel the marked text before the text view treats
        // Return/Escape/arrow keys as normal editing commands. Korean IME can
        // otherwise commit the syllable and also insert a newline on Return.
        if hasMarkedText() {
