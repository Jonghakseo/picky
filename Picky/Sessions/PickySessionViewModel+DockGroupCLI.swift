...
extension PickySessionListViewModel {
    ...
    func manageDockGroups(_ request: PickyDockGroupManagementRequest) throws -> [PickyDockGroupPayload] {
        ...
        case .create:
            ...
            _ = try dockLayoutController.createGroupPersisting(name: name, withMemberIDs: request.sessionIds)
        ...
        case .addMembers:
            ...
            guard try dockLayoutController.addSessionsPersisting(request.sessionIds, toGroup: group.id) else { break }
            dockLayout = dockLayoutController.layout
        case .removeMembers:
            let group = try PickyDockGroupCLIPolicy.validatedGroup(id: request.groupId, in: dockLayout)
            try PickyDockGroupCLIPolicy.validateMembers(request.sessionIds, in: group)
            guard try dockLayoutController.removeSessionsFromGroupPersisting(request.sessionIds) else { break }
            dockLayout = dockLayoutController.layout
        case .removeGroup:
            let group = try PickyDockGroupCLIPolicy.validatedGroup(id: request.groupId, in: dockLayout)
            _ = try dockLayoutController.removeGroupPersisting(id: group.id, keepMembers: true)
            dockLayout = dockLayoutController.layout
        case .archiveGroup:
            let group = try PickyDockGroupCLIPolicy.validatedGroup(id: request.groupId, in: dockLayout)
            ...
            _ = try dockLayoutController.removeGroupPersisting(id: group.id, keepMembers: false)
            dockLayout = dockLayoutController.layout
