...
const untilExtPath = path.join(home, ".pi", "agent", "extensions", "until", "index.ts");
const publishedExtPath = path.join(
  home,
  ".nvm",
  "versions",
  "node",
  ...
  "node_modules",
  "@ryan_nookpi",
  "pi-extension-idle-screensaver",
  "index.ts",
);
const daemonServerPath = path.join(process.cwd(), "src", "server.ts");
...
describe("classifyExtensionCrash", () => {
  ...
  it("identifies a TypeError thrown from a local pi extension folder", () => {
    ...
    error.stack = [
      ...
      `    at updateFooter (${untilExtPath}:239:24)`,
      `    at scheduleNext (${untilExtPath}:330:5)`,
      "    at Timeout.<anonymous> (node:internal/timers:594:17)",
    ].join("\n");
    expect(classifyExtensionCrash(error)).toEqual({
      extension: "until",
      extensionFrame: `at updateFooter (${untilExtPath}:239:24)`,
    });
  });
  ...
  it("identifies an error originating in a published pi-extension-* npm package", () => {
    ...
    error.stack = [
      "Error: boom",
      `    at showScreensaver (${publishedExtPath}:68:22)`,
      "    at Timeout.<anonymous> (node:internal/timers:594:17)",
    ].join("\n");
    expect(classifyExtensionCrash(error)).toEqual({
      extension: "pi-extension-idle-screensaver",
      extensionFrame: `at showScreensaver (${publishedExtPath}:68:22)`,
    });
  });
...
describe("installExtensionCrashGuard", () => {
  ...
  it("rethrows unhandledRejection that does not originate from extension code", () => {
    ...
    error.stack = [
      ...
      `    at handler (${daemonServerPath}:99:9)`,
    ].join("\n");
