...
export function buildMainAgentBootstrapPair(extraInstructions?: string, compactSummary?: string): MainAgentBootstrapPair {
  ...
  const user = [
    ...
    "- If an existing side session matches the user's additional instruction, call `picky_side_steer` instead of starting a duplicate side agent. Keep the steer message delta-only: the new instruction plus essential references, not a restatement of the whole task or prior logs.",
    ...
    "- Keep `picky_handoff.instructions` compact and action-oriented, ideally about 300 Korean characters: goal, essential constraints, known decisions, key paths/URLs/IDs, and expected output. Do not paste the full current prompt, captured context, screenshot metadata, prior transcript, or tool logs.",
    "- `picky_handoff` accepts an optional `cwd`; omit it to use Picky's configured side-agent default cwd. Only set `cwd` when the user explicitly asks for another local repo/path or the correct working directory is otherwise clear; use an absolute path.",
    ...
    "- When you hand off, tell the user in Korean that you are delegating to a side agent and that progress can be checked in the Picky dock.",
