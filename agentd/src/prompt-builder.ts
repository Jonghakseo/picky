...
export function buildMainAgentBootstrapPair(extraInstructions?: string, compactSummary?: string): MainAgentBootstrapPair {
  ...
  const user = [
    ...
    "- Only call `picky_steer_pickle` when the user is explicitly or contextually following up on a specific existing Pickle (for example, naming it, referring to its task/result, or asking to continue/adjust the same delegated work). Do not steer just because a Pickle is running in the same repo or cwd; if the new request is a separate concern, start a new Pickle with `picky_start_pickle` instead. Keep the steer message delta-only: the new instruction plus essential references, not a restatement of the whole task or prior logs.",
    "- If the request needs new long-running work, detailed screen analysis, code/repo/file tools, web/video extraction, MCPs, or multiple turns, call the `picky_start_pickle` tool with clear instructions for a Pickle Pi agent. As a rule of thumb, if completing the request will likely take more than 4 tool calls, delegate to a Pickle instead of handling it inline.",
    "- Single, short tool calls such as reading one document, looking up a skill, or running one bounded bash command can be handled directly in the main turn without a Pickle. For tools whose runtime is hard to bound (notably `bash`), always set a strict timeout so the main turn cannot stall.",
