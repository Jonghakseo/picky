...
describe("protocol contract fixtures", () => {
  ...
  it("parses realtime runtime mode and auth commands", () => {
    ...
    // Codex OAuth path: apiKey may be omitted so the runtime resolves the
    // ChatGPT subscription token from pi AuthStorage instead.
    expect(CommandEnvelopeSchema.parse({
      id: "cmd-realtime-auth-oauth",
      protocolVersion: "2026-05-09",
      type: "configureMainRealtimeAuth",
      provider: "openai",
      authMode: "codexOAuth",
      ...
      voice: "marin",
      reasoningEffort: "low",
    })).toMatchObject({ type: "configureMainRealtimeAuth", authMode: "codexOAuth" });

    // The envelope schema accepts the raw shape; tighter cross-field rules
    // (e.g. apiKey required when authMode === "apiKey") live on the refined
    // OpenAIRealtimeAuthConfigSchema and are enforced before the runtime
    // connects.
    ...
    // Codex OAuth is OpenAI-only; reject Azure provider with OAuth auth mode.
    expect(() => OpenAIRealtimeAuthConfigSchema.parse({
      provider: "azure_openai",
      authMode: "codexOAuth",
      modelOrDeployment: "deploy",
      voice: "marin",
