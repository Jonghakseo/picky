...
export function buildRealtimeInstructions(
  ...
  const baseLines = [
    ...
    "- When the user grounds a request in what they are currently looking at (\"this page\", \"이 페이지\", \"이 화면\", \"여기\", \"이거\", \"this PR\", \"이 이슈\"), copy the concrete reference from the captured-context block verbatim into the Pickle `instructions` so the Pickle does not lose the target. Specifically: for a browser turn, paste the exact `- Browser URL:` value (and the browser title when useful) into `instructions`; for a non-browser app, name the active app and window title; when the user pointed at a screenshot region or drew an ink mark, describe what is shown in that region in words (Pickle does not receive your screenshots). Never paraphrase or shorten a URL, issue id, PR number, file path, or commit hash — pass it through exactly as captured. Do the same for `picky_steer_pickle` follow-ups.",
    "- If the user references the current screen but the captured-context block has no usable URL / window title / selection, ask one short clarifying question instead of guessing or sending a Pickle without the target.",
    "",
    "## Reasoning",
