...
export class SessionMessageBuilder {
  ...
  async recordUserText(
    ...
    await this.appendInternal(sessionId, {
      ...
      originatedBy,
      text: trimmed,
      ...(attachedImagesCount !== undefined ? { attachedImagesCount } : {}),
    });
  }
  ...
  async recordExtensionQuestion(sessionId: string, request: PickyExtensionUiRequest): Promise<void> {
    ...
    const cancelledAt = state.cancelledIds.has(request.id) ? this.deps.now() : undefined;
    state.cancelledIds.delete(request.id);
    await this.appendInternal(sessionId, {
      id: request.id,
      kind: "agent_question",
      createdAt: this.deps.now(),
  ...
  async recordExtensionNotification(sessionId: string, request: PickyExtensionUiRequest): Promise<void> {
    const text = extensionNotifyText(request);
    if (!text) return;
    await this.appendInternal(sessionId, {
      id: request.id,
      kind: "system",
  ...
  async cancelExtensionQuestion(sessionId: string, requestId: string): Promise<void> {
    ...
    if (!entry) {
      if (!state.removedIds.has(requestId)) state.cancelledIds.add(requestId);
      return;
    }
    if (state.removedIds.has(requestId) || entry.message.cancelledAt) return;
  ...
  async upsertSystemMessage(sessionId: string, messageId: string, text: string): Promise<void> {
    await this.enqueue(sessionId, async () => {
      const state = this.stateFor(sessionId);
      const existing = state.journal.find((entry) => entry.message.id === messageId);
      const message: PickySessionMessage = {
        id: messageId,
        kind: "system",
        createdAt: existing?.message.createdAt ?? this.deps.now(),
        text,
      };
      if (existing) {
        await this.replaceInternal(sessionId, messageId, message);
      } else {
        await this.appendInternal(sessionId, message);
      }
    });
  }
  ...
  private stateFor(sessionId: string): SessionState {
    ...
    const state: SessionState = { journal: [], removedIds: new Set(), cancelledIds: new Set(), assistantDraft: "", thinkingDraft: "" };
...
function extensionNotifyText(request: PickyExtensionUiRequest): string | undefined {
  const text = request.prompt ?? request.text ?? request.title;
  const trimmed = text ? stripAnsiEscapeSequences(text).trim() : undefined;
