...
### 3. Pre-create the `auto-update` tag
...
1. Push the version tag (e.g. `v1.0.5`) and let
   `.github/workflows/beta-notarized-release.yml` run, **or** trigger it via
   `workflow_dispatch` with `release_channel=stable` for a stable build.
2. The workflow:
   ...
   - Updates `appcast.xml` (downloaded from the `auto-update` release, new
     `<item>` prepended, then re-uploaded with `gh release upload --clobber`).
...
2. Pointing `SUFeedURL` at a local file via `defaults` for the running app:
   ```bash
   defaults write com.creatrip.Picky SUFeedURL "file:///tmp/test-appcast.xml"
   ```
