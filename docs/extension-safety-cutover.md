...
- PTT abort reuses its main handle and pauses external cron delivery until replacement speech is accepted. A per-runtime Pi EventBus carries `picky.external-delivery.pause-state` and `picky.external-delivery.pause-query`; reload queries the current pause state, and cron returns `deferred` before queuing input while paused. Cancelling recording without a replacement prompt leaves delivery paused until the next input or runtime replacement.
- Terminal-sync invalidation waits for disposal before reopening the same JSONL. Failed teardown blocks that resume instead of creating an overlapping writer.
