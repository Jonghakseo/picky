...
## Full-row disclosures and consistent settings spacing

Settings Details, advanced settings, and archived sessions now share `PickySettingsDisclosureStyle`. Its standard SwiftUI Button owns the entire row (including label and whitespace), has a minimum 32pt height, hides the decorative chevron from accessibility, and exposes localized collapsed/expanded values. Expanded content occupies the full width with leading multiline alignment. There is no new motion dependency. Live mouse, keyboard, and VoiceOver activation have not been exercised; the offscreen gallery is not evidence for those interactions.

Label/control/help groups use 8pt spacing; independent fields use 16–24pt separation. Voice/provider/Edge TTS warnings, Pickle controls, shortcuts, tool descriptions, and compact settings buttons were reviewed together. Dock-size and Pi/Shell segmented choices now have the same bounded width as other embedded selectors and regular control sizing. Values, draft ownership, and save handlers remain unchanged.

The Hub gallery and three menu/persistence regressions passed. The optional exported-snapshot test did not exercise its absent-request path. Expanded disclosure light/dark component renders supplement the production full-page scenes; they confirm wrapping and left alignment, not clicking. Token lint and diff checks passed. The app was not restarted.

The targeted `toolsDeepLinkMountsSettingsAndExpandsBuiltinToolsOnFirstVisit()` runtime regression also passed. It navigates through the production settings path and observes rendered built-in tool text after expansion, confirming that the shared style preserves expanded-content appearance/navigation delivery. This remains separate from unverified physical button/keyboard/VoiceOver activation.
