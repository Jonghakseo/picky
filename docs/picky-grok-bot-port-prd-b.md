...
- 개정 기준: B 0.6 `6a2aaca7c` 이후의 루틴 히스토리 누락 피드백. 실행 내역의 진입·열람·당시 정의 확인을 추가한다. 기존 내장 런타임·메신저·로컬 컴퓨터 설계는 유지한다. 별도 디자인 시스템·컴포넌트 HTML은 사용자 검수 전 목업에 적용하지 않는다.
...
- 루틴 목록과 편집 화면에서 히스토리를 연다. 원 Bot·Routine의 실행별 시각·trigger·상태·당시 revision·결과를 보여주며, 현재 정의를 과거 실행으로 표시하지 않는다. 수락·실행·완료·실패·불명 결과를 구별하고, 기록 열람은 재실행·새 turn·설정 저장을 하지 않는다. 현재 열람 권한과 원 결과 링크의 접근 범위를 재검사한다.
...
`모의 실행 기록`을 남기고, 컴퓨터는 `설정 필요`로 유지한다. 이는 제품 실행·격리의 완료 근거가 아니다. B.06 히스토리는 최근 모의 기록과 저장 당시 정의 사본을 보여주며, 이전 기록에 없는 정의나 실제 실패·성공을 추정하지 않는다.
...
| 루틴 히스토리 열람·정의 수정·재접속 | 다른 피클 기록 노출, 최신 지침으로 과거 실행 덮어쓰기, 열람으로 재실행, 실제 상태·결과 조작 |
...
실제 Grok 로그인 계정, OpenMaus packaged app, Computer·connector·Routine 통합을 실행하지 않았다. VM 설치, Picky 앱 재시작, 제품 build/test도 수행하지 않았다. B 0.7은 기존 설계를 유지하고 루틴 히스토리 열람을 보완한 문서 개정이다. HTML 목업 검증은 실제 내장화·격리·migration·권한 실행의 검증이 아니다. 이 문서의 수용 표는 향후 검증 계획이다.
