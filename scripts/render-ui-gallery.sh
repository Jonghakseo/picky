...
if [ "$TARGET" = "conversation-activity" ]; then
  ...
  REQUEST_FILE="$ROOT/build/render-gallery/.conversation-activity-output-path"
  EXPECTED=(
    activity-summary-collapsed-dark-ko.png
    activity-summary-expanded-dark-ko.png
    activity-summary-collapsed-light-ko.png
    activity-summary-expanded-light-ko.png
  )
  ...
  echo "Rendering conversation-activity gallery offscreen to $OUTPUT"
  xcodebuild -project Picky.xcodeproj -scheme Picky -destination "$DESTINATION" \
    test -only-testing:PickyTests/PickyActivitySummaryRenderGalleryTests
...
print(f"Validated {len(expected)} conversation-activity PNG renders.")
PY
...
if [ "$TARGET" = "conversation-composer" ]; then
...
print(f"Validated {len(expected)} conversation-composer PNG renders.")
PY
